...
class TranslatorMiddleware {
    ...
    async onTurn(turnContext, next) {
        if (turnContext.activity.type === 'message') {
            ...
            if (shouldTranslate) {
                turnContext.activity.text = await this.translate(turnContext.activity.text, userLanguage);
            }
            ...
            turnContext.onSendActivities(async (context, activities, next) => {
                ...
                if (shouldTranslate) {
                    for (const activity of activities) {
                        activity.text = await this.translate(activity.text, DEFAULT_LANGUAGE);
                    }
                }
                await next();
            });
    ...
    async translate(text, to) {
        ...
        // const url = 'https://api.cognitive.microsofttranslator.com/translate?api-version=3.0&to='+to;
		 ...
		 const url = 'https://api.cognitive.microsofttranslator.com/translate?api-version=3.0&to=hi';
        ...
        return fetch(url, {
            ...
            headers: {
                ...
                'Ocp-Apim-Subscription-Key': 'e93a5fd4ef224c13a219b68bce70bdde'
            }
        })
            .then(res => res.json())
            .then(jsonResponse => {
